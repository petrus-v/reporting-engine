...
class IrQWeb(models.AbstractModel, QWeb):
    ...
    def _render(self, id_or_xml_id, values=None, **options):
        ...
        if b"data-page-break=" not in result:
            return result
    ...
    def _split_tables(self, html_content):
        ...
        for fragment in html_document:
            table = None
            for row in fragment.iterfind(".//tr[@data-page-break]"):
                # while generating pdf this method is called twice so we prevent
                # table to be split twice by removing data-page-break once
                # table has been spited
                pos = row.attrib.pop("data-page-break")
    ...
    @api.model
    def _set_page_break_style(self, table, force_break):
